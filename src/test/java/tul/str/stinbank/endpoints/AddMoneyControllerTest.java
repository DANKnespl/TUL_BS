...
public class AddMoneyControllerTest {
    ...
    public void addMoney() throws Exception{
        ...
        money.add(0f);
        money.add(0f);
        Account account = new Account("testAdd",money,curr,history);
        ...
        assertEquals(account.getMoney().get(0),21.1,0.01);
        mockMvc.perform(get("/add")
               .param("amount", "10")
               .param("abr", "USD")
               .param("userID", "0"))
        ...
        mockMvc.perform(get("/add")
               ...
               .param("userID", "0"))
               .andExpect(status().isOk());
        assertEquals(account.getMoney().get(0),256.1,0.01);
        assertEquals(account.getMoney().get(1),10,0.01); 
        ...
        if("EUR".equals(account.getHistory().get(account.getHistory().size()-1).getAbr())){
            //assertEquals(account.getMoney().get(1),20,0.01);
        }else{
            //assertNotEquals(account.getMoney().get(0),256.1,0.01);
        }
